import {
  ...
  TouchableOpacity,
  Vibration,
} from "react-native"; // Added Vibration
...
import { medications, logs, appSettings, doses } from "@/core/database/schema";
...
import { cancelNotification } from "@/utils/notifications"; // Import
...
export default function Dashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (settingsResult.length > 0) {
        setUserSettings({
          showDaysSupply: settingsResult[0].showDaysSupply ?? true,
        });
        // LOAD NEW VALUES
  ...
  const handleTakePill = async (
    ...
    if (currentStock <= 0) {
      showAlert({
        title: "Out of Stock",
        message: "You have no pills left! Please restock.",
        variant: "warning",
      });
      return;
    }
    ...
    try {
      ...
      console.error("Failed to take pill:", error);
    }
  };
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              const medDoses = await db
                .select()
                .from(doses)
                .where(eq(doses.medicationId, med.id));

              for (const dose of medDoses) {
                if (dose.notificationId) {
                  // Check if it's a JSON array (Weekly) or single string (Daily)
                  if (dose.notificationId.startsWith("[")) {
                    const ids = JSON.parse(dose.notificationId);
                    ids.forEach((id: string) => cancelNotification(id));
                  } else {
                    cancelNotification(dose.notificationId);
                  }
                }
              }
    ...
    if (menuType === "ADMIN") {
      return [
        ...
        },
        {
          text: "Cancel",
          colorType: "cancel",
          icon: <X size={iconSize} color={cancelColor} />,
    ...
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="absolute bottom-8 right-6">
        <TouchableOpacity
          ...
          className="bg-blue-600 w-16 h-16 rounded-full items-center justify-center shadow-xl"
        >
      ...
      <PopUpCard
        ...
        title={
          menuType === "ADMIN" ? `Manage ${selectedMed?.name}` : "Dose Options"
        }
        message={
          menuType === "ADMIN"
            ? "Inventory & Settings"
            : "Did you miss or lose this pill?"
        }
