...
export default function AddMedicine() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-900" edges={['top']}>
      ...
      <ScrollView className="flex-1 p-5" contentContainerStyle={{ paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
            ...
            <View className="flex-row gap-3 -z-10">
              {(["nos", "mg", "ml"] as const).map((u) => (
                <TouchableOpacity
                  ...
                  style={{ backgroundColor: unit === u ? '#2563EB' : (isDark ? '#374151' : '#F9FAFB'), borderColor: unit === u ? '#2563EB' : (isDark ? '#4B5563' : '#E5E7EB'), borderWidth: 1 }}
                  className="px-4 py-2 rounded-full"
                >
                  <Text style={{ color: unit === u ? 'white' : (isDark ? '#D1D5DB' : '#4B5563') }} className="font-bold capitalize">{u === "nos" ? "Pill" : u}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
        ...
        <View className="mb-6 -z-10">
          ...
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 flex-row items-center justify-between">
            <View>
              <Text className="text-lg font-bold text-gray-900 dark:text-white">Current Stock</Text>
            ...
            <View className="flex-row items-center bg-gray-100 dark:bg-gray-700 rounded-xl px-4 py-2">
              <Hash size={20} color={isDark ? '#9CA3AF' : '#6B7280'} className="mr-2" />
              <TextInput value={currentStock} onChangeText={setCurrentStock} keyboardType="numeric" placeholder="0" placeholderTextColor="#9CA3AF" className="text-2xl font-bold text-gray-900 dark:text-white w-16 text-center" />
            </View>
          </View>
        </View>
          ...
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
            <View className="flex-row bg-gray-100 dark:bg-gray-700 p-1 rounded-xl mb-6">
              <TouchableOpacity onPress={() => setFrequency("DAILY")} className="flex-1 py-2 rounded-lg items-center" style={{ backgroundColor: frequency === 'DAILY' ? (isDark ? '#4B5563' : 'white') : 'transparent', shadowOpacity: frequency === 'DAILY' ? 0.1 : 0 }}>
                <Text style={{ color: frequency === 'DAILY' ? (isDark ? '#60A5FA' : '#2563EB') : '#9CA3AF' }} className="font-bold">Every Day</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setFrequency("WEEKLY")} className="flex-1 py-2 rounded-lg items-center" style={{ backgroundColor: frequency === 'WEEKLY' ? (isDark ? '#4B5563' : 'white') : 'transparent', shadowOpacity: frequency === 'WEEKLY' ? 0.1 : 0 }}>
                <Text style={{ color: frequency === 'WEEKLY' ? (isDark ? '#60A5FA' : '#2563EB') : '#9CA3AF' }} className="font-bold">Specific Days</Text>
              </TouchableOpacity>
            </View>

            <View className="flex-row justify-between mb-6" style={{ display: frequency === 'WEEKLY' ? 'flex' : 'none' }}>
                {["S", "M", "T", "W", "T", "F", "S"].map((day, index) => {
                  const isSelected = selectedDays.includes(index);
                  return (
                    <TouchableOpacity key={index} onPress={() => toggleDay(index)} className="w-10 h-10 rounded-full items-center justify-center border" style={{ backgroundColor: isSelected ? '#2563EB' : 'transparent', borderColor: isSelected ? '#2563EB' : (isDark ? '#4B5563' : '#D1D5DB') }}>
                      <Text className="font-bold" style={{ color: isSelected ? 'white' : '#6B7280' }}>{day}</Text>
                    </TouchableOpacity>
                  );
                })}
            </View>
            ...
            <View>
              <Text className="text-gray-500 dark:text-gray-400 text-xs font-bold uppercase mb-3">Times & Dosage</Text>
              {doseList.map((dose, index) => (
                <View key={index} className="flex-row items-center mb-3 gap-3">
                  <TouchableOpacity onPress={() => setShowPicker({ visible: true, index })} className="flex-1 flex-row items-center bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 p-3 rounded-xl">
                    <Clock size={20} color={isDark ? '#60A5FA' : '#2563EB'} className="mr-2" />
                    <Text className="text-lg font-bold text-gray-900 dark:text-white">{formatTime(dose.time)}</Text>
                  </TouchableOpacity>
                  <View className="flex-row items-center bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 p-3 rounded-xl w-24">
                    ...
                    <TextInput value={dose.qty} onChangeText={(txt) => { const newList = [...doseList]; newList[index].qty = txt; setDoseList(newList); }} keyboardType="numeric" className="text-lg font-bold text-gray-900 dark:text-white flex-1 text-center" />
                  </View>
                  {doseList.length > 1 && (
                    <TouchableOpacity onPress={() => removeDoseRow(index)} className="p-3 bg-red-50 dark:bg-red-900/30 rounded-xl">
                      <Trash2 size={20} color="#EF4444" />
                    </TouchableOpacity>
                  )}
                </View>
              ))}
              <TouchableOpacity onPress={addDoseRow} className="flex-row items-center justify-center py-3 mt-2 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl">
                <Plus size={20} color="#6B7280" />
                <Text className="text-gray-500 font-bold ml-2">Add Another Dose Time</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      {showPicker.visible && (
        <DateTimePicker value={doseList[showPicker.index]?.time || new Date()} mode="time" display={Platform.OS === "ios" ? "spinner" : "default"} onChange={updateDoseTime} />
      )}
    </SafeAreaView>
  );
}